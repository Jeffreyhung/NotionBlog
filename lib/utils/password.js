...
export const getPasswordQuery = path => {
  ...
  const queryPasswordHash = queryParams.password
    ? sha256Digest(queryParams.password)
    : ''

  // 请求中带着密码（仅存摘要，避免明文落盘）
  if (queryPasswordHash) {
    // 将已输入密码摘要作为默认密码存放在 localStorage，便于下次读取并自动尝试
    localStorage.setItem('password_default', queryPasswordHash)
  }

  // 获取路径部分
  const cleanedPath = url.pathname

  // 从 localStorage 中获取相关密码（统一为摘要；兼容旧明文并迁移）
  const normalizeToHash = value => {
    if (!value) return ''
    return isSHA256Digest(value) ? value : sha256Digest(value)
  }
  const storedPassword = normalizeToHash(
    localStorage.getItem('password_' + cleanedPath)
  )
  const defaultPassword = normalizeToHash(localStorage.getItem('password_default'))

  // 将所有候选密码（摘要）存储在一个数组中，并过滤掉无效值
  const passwords = [queryPasswordHash, storedPassword, defaultPassword].filter(
    Boolean
  )
