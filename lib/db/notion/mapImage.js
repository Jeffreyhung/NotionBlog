 ...
 * @returns
 */
const isTrustedNotionStaticHost = (urlString) => {
  try {
    const parsed = new URL(urlString)
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') return false
    const host = parsed.hostname
    return host === 'secure.notion-static.com' || host.endsWith('.secure.notion-static.com')
  } catch (e) {
    return false
  }
}

const isNotionImagesUrl = (urlString) => {
  try {
    const parsed = new URL(urlString)
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') return false
    return parsed.hostname === 'www.notion.so' && parsed.pathname.startsWith('/images/')
  } catch (e) {
    return false
  }
}
